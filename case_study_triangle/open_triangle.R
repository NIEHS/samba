open_triangle <- function(
  ...
  stopifnot(
    "current dir is not in case_study_triangle" =
      basename(ifelse(is.null(path), getwd(), path)) == "case_study_triangle"
  )
  data <- list()
  ...
  if (pred == TRUE) {
    data$pred <- readRDS(
      paste0(
        path,
        "./input/grid_full_with_covar_202107.rds"
      )
    )
  }
  ...
  if (covariates == TRUE) {
    ...
    modis_et_path <- paste0(
      path,
      "../input/MOD16A3GF.A2021001.h11v05.061.2022024075249.hdf"
    )
  ...
  if (ref_network == TRUE) {
    source(paste0(path, "open_econet.R"))
    data$ref <- open_econet(f_path = paste0(path, "./input/econet/"))
  }
  return(data)
}
