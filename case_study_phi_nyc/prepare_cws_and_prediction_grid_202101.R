...
pts_era5 <- extract_era5(
    ...
    path_accum,
    path_instant,
    ts = as.POSIXct("2021-01-01 00:00:00", tz = "UTC"),
    te = as.POSIXct("2021-01-31 23:00:00", tz = "UTC")
  )
pts_covar <- merge(
...
# Save final dataset
saveRDS(cws_covar, "./input/cws/cws_covar_2021-01.rds")
write.csv(cws_covar, "./input/cws/cws_covar_2021-01.csv")
...
for (id in cells_id) {
  ...
  pts_covar$local_hour <- lubridate::with_tz(
    ...
    lubridate::hour()
  saveRDS(pts_covar,
    file = paste0("./input/grid_", id, "_with_covar_202101.rds")
  )
  write.csv(pts_covar,
    file = paste0("./input/grid_", id, "_with_covar_202101.csv")
  )
...
for (i in seq_along(cells)) {
  pred_list[[i]] <- readRDS(
    paste0("./input/grid_", cells[i], "_with_covar_202101.rds")
  )
}
...
saveRDS(pred,
  file = "./input/grid_full_with_covar_202101.rds"
)
write.csv(pred,
  file = "./input/grid_full_with_covar_202101.csv"
)