# Geographical context and meteorological conditions
...
data <- open_phi_nyc(cws = TRUE, covariates = TRUE, ref_network = TRUE)
...
ref_locs <- ref_locs |>
  sf::st_transform(terra::crs(data$lcz))
ref_locs$lon <- sf::st_coordinates(ref_locs)[, 1]
ref_locs$lat <- sf::st_coordinates(ref_locs)[, 2]
p_lcz <- ggplot() +
  ...
  ggplot2::geom_text(
      data = ref_locs,
      aes(x = lon, y = lat + 3000, label = site_id),
      size = 3
    ) +
...
p_all <- ggpubr::ggarrange(p_bf, p_imp, p_tcc, p_fch, p_elev, p_lcz,
  nrow = 2, ncol = 4)
ggsave(
  plot = p_all,
