...
#' @author Eva Marques
#' @export
inference_mod2 <- function(data, pred, polygon, ts, te, verbose = FALSE) {
  stopifnot(
  ...
  a_st_imp <- INLA::inla.spde.make.A(
    mesh = mesh_s,
    loc = cbind(data$lon, data$lat),
    group = data$timeidx,
    weights = data$imp
  ...
  ap_st_imp <- INLA::inla.spde.make.A(
    mesh = mesh_s,
    loc = cbind(pred$lon, pred$lat),
    group = pred$timeidx,
    weights = pred$imp
  ...
  s_idx_imp <- INLA::inla.spde.make.index("s_imp",
    n.spde = m,
    n.group = nrow(timedim)
  )
  # data wrapper
  stk_data <- INLA::inla.stack(
    tag = "data",
    data = list(y = data$temp_cal),
    A = list(1, a_st, a_st_imp_hour),
    effects = list(
      data.frame(
        int = rep(1, nrow(data)),
        era5 = data$era5,
  ...
  formula <- y ~ -1 + int + era5 + elev + tcc + fch + bf +
    f(s,
